...
int main(int argc, char *argv[])
{
	/// 输入参数定义
	if (argc < 2)
		fprintf(stderr, "Equinox Version %d\n", PROJECT_VERSION_FULL);
	...
	int this_OCS = 0;
	//int duty_schedule[NUM_OCS << 1];
	//for (index = 0; index < NUM_OCS; ++index)		index[duty_schedule] = ((NUM_OCS << 1) - 1 - index)[duty_schedule] = index; /// 生成蛇形循环轮值表
	int occupied_slot_branch[NUM_OCS] = {0}; /// 每个窗口都从0开始迭代
	int mark = 0;
	...
	thisturn_arrival_count = fine_poisson_arrival(demand_queue, window, &thisturn_arrival_product);
	//print_matrix(demand_queue, NUM_PORT, NUM_PORT);

	for (iter = 1; iter <= epoch; ++iter)
	{
		...
		total_arrival_product += thisturn_arrival_product; /// 以本轮开始为0点，上一轮到达平均约为一半window处，相差的一轮最后在时间上补足
		...
		memset(occupied_slot_branch, 0, NUM_OCS * sizeof(int));
		while ((this_avail_slot = window - RECONFIGURATION_DELAY - minimum_element_value(occupied_slot_branch, NUM_OCS)) > 0)
		{
			this_OCS = minimum_element_id(occupied_slot_branch, NUM_OCS);
			...
			simulated_slot = maximum_element_value(occupied_slot_branch, NUM_OCS) + last_checkpoint;
		}
		last_checkpoint = (window * iter);
		simulated_slot = last_checkpoint;

		last_remain = (long long)sum_item(demand, NUM_PORT * NUM_PORT);
		fprintf(stderr, "Demand remain in this turn: %d\n", last_remain);

		if (last_remain)
		{
			//fprintf(stderr, "\nTraffic not clean at epoch %d, timeout\n", iter);
			//return 1;
			/// 未清空的写回到达队列，注意轮空罚时
			for (index = 0; index < NUM_PORT * NUM_PORT; ++index)
				if (index[demand])
			...
			total_departure_product += last_remain * window; /// 小心此处乘法溢出
		}
		...
		/// calc_traffic_delay;
		mean_delay = (REAL)(total_departure_product - total_arrival_product) / total_arrival_count + window;
		if ((mean_delay <= 0))
		{
		...
		frequency_reconfig = total_reconfiguration_counter / simulated_slot_ms; /// unit: ms^(-1)
